...
def main(args):
    ...
    if args.model == 0:
        ...
        model = RNNWithAttention( input_dim= 100, hidden_dim= 128 ,lstm=True).cuda()
    ...
    for epoch in range(args.epoch):
        ...
        test_f1 = f1_score(y, output)

        for i in range(len(y)):
            y[i] = 1- y[i]
            output[i] = 1 - output[i]
        test_f0 = f1_score(y,  output)
        ...
        valid_f1 = f1_score(y, output)
        for i in range(len(y)):
            y[i] = 1- y[i]
            output[i] = 1 - output[i]
        valid_f0 = f1_score(y, output)

        print('Epoch %2d: loss: %10.2f > Test loss: %5.2f acc: %0.4f f1s: %0.4f f0s: %0.4f > Valid loss: %5.2f acc: %0.4f f1s: %0.4f f0s: %0.4f '
              % (epoch, train_loss, test_loss, test_acc, test_f1 , test_f0 , valid_loss, valid_acc , valid_f1 , valid_f0), file=log)
        print()
        print('Epoch %2d: loss: %10.2f > Test loss: %5.2f acc: %0.4f f1s: %0.4f f0s: %0.4f > Valid loss: %5.2f acc: %0.4f f1s: %0.4f f0s: %0.4f '
            % (epoch, train_loss, test_loss, test_acc, test_f1, test_f0, valid_loss, valid_acc, valid_f1, valid_f0))
